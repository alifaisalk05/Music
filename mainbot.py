...
class HealthHandler(BaseHTTPRequestHandler):
    """Minimal HTTP health endpoint for Render and UptimeRobot."""

    def do_GET(self):
        if self.path.rstrip("/") == "/health":
            body = b'{"status":"ok","service":"telegram-music-bot"}'
        ...
        body = b"Telegram Music Bot is running"
        self.send_response(200)
...
async def live_stream(_, message: Message, lang):
    ...
    if " " in args and args.count(" ") == 1 and args[-5:] == "parse":
        ...
        if is_yt_url:
            ...
            ytstreamlink = next(
                (item.get("url") for item in formats if item.get("url")),
                None,
            )
            if not ytstreamlink:
                raise ValueError("No playable stream URL was found.")
            link = ytstreamlink
            song = Song(
...
if __name__ == "__main__":
    print("Starting Telegram Music Player...")
    run_health_server()
    client.start()
    print("Telegram client started. Starting voice-call engine...")
    pytgcalls.run()
